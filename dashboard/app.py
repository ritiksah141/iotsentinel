...
def save_device_changes(n_clicks, ip, name, device_type, group_id, notes):
    ...
    try:
        ...
        toast = dbc.Toast(
            f"Device '{name or ip}' updated successfully!",
        ...
        return False, load_device_management_table(1), toast
    except Exception as e:
        logger.error(f"Error saving device changes: {e}")
        toast = dbc.Toast(
            f"Failed to update device: {str(e)}",
...
@app.callback(
    [Output('page-content', 'children'),
     Output('auth-notification-store', 'data')],
    [Input('url', 'pathname')],
    prevent_initial_call=False
)
