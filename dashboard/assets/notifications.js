...
class NotificationManager {
  ...
  connectEventStream() {
    ...
    try {
      ...
      this.eventSource = new EventSource(
        `/notifications/stream?client_id=${clientId}`
      );
  ...
  showNotification(notification) {
    ...
    if (
      ...
      notification.type === "rule_triggered"
    ) {
      options.actions = [
        {
          action: "view",
  ...
  addToQueue(notification) {
    ...
    try {
      localStorage.setItem(
        "iotsentinel_notification_queue",
  ...
  loadSettings() {
    try {
      const settings = JSON.parse(
        localStorage.getItem("iotsentinel_notification_settings") || "{}"
      );
      this.enabled = settings.enabled || false;

      // Load queue
      const queue = JSON.parse(
        localStorage.getItem("iotsentinel_notification_queue") || "[]"
      );
  ...
  saveSettings() {
    try {
      ...
      localStorage.setItem(
        "iotsentinel_notification_settings",
...
// Dash clientsideside function to close notification drawer
window.dash_clientside = window.dash_clientside || {};
window.dash_clientside.clientside = window.dash_clientside.clientside || {};

window.dash_clientside.clientside.close_notification_drawer = function (
  n_clicks,
  is_open
) {
  if (n_clicks > 0 && is_open) {
    return false;
  }
