...
| Metric | Value |
|---|---|
| Total tests | **1247 passing**, 9 skipped, 0 failing |
| Test files | 52 |
...
#### `test_db_resilience.py` - 31 tests
**Covers:** `database/db_manager._connect()` degrade-not-crash behaviour, `utils/net_detect` gateway/host helpers, infrastructure-device seeding, log rotation, the systemd / journald disk-safety guards, the self-signed-cert generator, HTTPS-serving wiring, the request-derived WebAuthn origin, the dashboard-unit capability sanity, the scheme-aware setup-wizard URLs, Tailscale Funnel HTTPS backend, and WebAuthn cert-error guidance.

**Why it exists:** a full or failing SD card made `PRAGMA journal_mode = WAL` raise "disk I/O error", which crashed the backend and dashboard at import and tripped systemd's start-limit, leaving the dashboard permanently unreachable. These tests pin the WAL->rollback-journal fallback (so the app comes up degraded instead of bricking), the stale-WAL cleanup, the gateway/host seeding that stops the dashboard showing "0/N online" at boot, the rotating log handlers, the `StartLimitIntervalSec=0` units, and the journald size cap. They also pin the HTTPS-on-LAN path (self-signed cert loads into an SSL context and covers the right SANs; cert/keyfile are passed to the server with a Secure-cookie downgrade on HTTP fallback) and that the WebAuthn ceremony follows the browser's real origin. Two guards prevent the rc13 setup-wizard regressions: the dashboard unit must carry **no** `CapabilityBoundingSet` line (it would cap the process tree to `CAP_NET_BIND_SERVICE` and strip sudo of `CAP_SETUID/SETGID/AUDIT_WRITE`, breaking the final-step Wi-Fi join and hotspot teardown), and the wizard's reach/handoff URLs must be scheme-aware `https` (HTTPS-on-LAN is default-on) rather than hardcoded `http`. Two rc15 guards prevent the Tailscale 502 regression (`_enable_tailscale_funnel` must use `https+insecure://` so Tailscale accepts the self-signed backend cert) and ensure the WebAuthn error handler surfaces a cert-specific message directing users to their Tailscale ts.net URL.
