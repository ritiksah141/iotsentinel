...
| FR-005  | US-005: Incremental Online Learning     | Incremental anomaly detection via River ML (HalfSpaceTrees + HoeffdingAdaptive). No offline training; models learn from live traffic from first connection. | `ml/river_engine.py`<br>`ml/inference_engine.py`                                                     | TC-RIVER-001 to TC-RIVER-023, TC-ML-001 to TC-ML-025 | ✅ Implemented         |
...
| FR-014  | US-008: User Experience & Accessibility | Onboarding / Interactive Tour                                          | `dashboard/shared.py:697-846`<br>`dashboard/app.py:8202-8217`<br>`dashboard/callbacks/callbacks_global.py:801-870` | TC-VAL-012                                    | ✅ Implemented         |
...
| FR-019  | US-013: Data Export (CSV) & Reporting   | Weekly Report Generation                                               | `scripts/generate_report.py:1-103`                                                                   | TC-VAL-014                                    | ✅ Implemented         |
| FR-020  | US-015: IoT Device Classification       | Device Type Detection & Icons                                          | `utils/device_classifier.py`<br>`database/db_manager.py:48-118`<br>`dashboard/app.py:372-389`        | TC-IOT-001                                    | ✅ Implemented         |
| FR-021  | US-016: Device Management               | Custom Names, Grouping, Notes                                          | `database/db_manager.py:380-469`<br>`dashboard/app.py:1887-1899,4138-4220`                           | TC-IOT-002                                    | ✅ Implemented         |
| FR-022  | US-017: Dashboard Preferences           | Settings Customization                                                 | `dashboard/app.py` (Settings callbacks & device metadata management)                                 | TC-UX-003                                     | ✅ Implemented         |
| FR-023  | US-018: IoT Security Assessment         | Security Widget & Checks                                               | `utils/iot_security_checker.py`<br>`dashboard/app.py:1660-1673,4290-4368`                            | TC-SEC-006                                    | ✅ Implemented         |
| FR-024  | US-019: Login Rate Limiting             | Brute Force Protection                                                 | `utils/rate_limiter.py`<br>`dashboard/app.py:43,66,3653-3694`                                        | TC-SEC-005                                    | ✅ Implemented         |
| FR-025  | US-020: Educational Chart Tooltips      | In-App Help System                                                     | `dashboard/app.py:1439-1451,1511-1542,1586-1628`                                                     | TC-UX-002                                     | ✅ Implemented         |
| FR-026  | US-021: Health Check Endpoint           | /health API Monitoring                                                 | `dashboard/app.py:88-155`                                                                            | TC-SYS-006                                    | ✅ Implemented         |
| FR-027  | US-022: Enhanced Deployment             | Backup, Rollback, Clean Install                                        | `scripts/deploy_to_pi.sh`<br>`.env.template`                                                         | TC-DEP-001                                    | ✅ Implemented         |
...
| --------------------------------- | ------ | ------------------------ |
| Functional Requirements (FR)      | 27     | ✅ 27 Implemented        |
| Non-Functional Requirements (NFR) | 4      | ✅ 4 Implemented         |
| WON'T HAVE Features               | 2      | ⚫ Excluded by design    |
| **Total Requirements**            | **31** | **31 Unique (100% complete)** |
...
- Unit Tests: TC-DB-\*, TC-ML-\*, TC-CAP-\*, TC-RIVER-\*
- Integration Tests: TC-INT-\*
- System Tests: TC-SYS-\*
- Validation Tests: TC-VAL-\*
- Security Tests: TC-SEC-\*
- **True baseline (2026-05-12)**: 182 tests collected — 170 pass, 11 skip (env-conditional), 1 xfail (deferred engine bug)
...
| Date | Change |
|---|---|
| 2026-05-12 | **Phase 0 RTM hygiene.** Removed redundant pointers FR-019 (was duplicate of FR-013), FR-021 (was duplicate of NFR-002), NFR-005 (was duplicate of NFR-004). Renumbered all requirements below each deletion. Rewrote FR-005 to describe current River ML incremental learning requirement (removed migration/deprecated language). Updated FR-005 and FR-014 code file references to correct line numbers. Updated test case references for FR-005 to match `test_river_ml.py` and `test_ml.py`. |
