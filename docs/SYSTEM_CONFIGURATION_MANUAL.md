...
IoTSentinel's **API Integration Hub** provides secure management of **28 free-tier external API integrations** across 5 categories. All credentials are encrypted using **AES-256** encryption before storage.

### Features

- **28 Free-Tier Integrations** across Threat Intel, Geolocation, Notifications, Ticketing, and Webhooks
- **Dashboard Configuration** - No `.env` editing required
- **Encrypted Storage** - Military-grade AES-256 encryption (Fernet)
- **Health Monitoring** - Real-time integration status tracking
- **Usage Analytics** - Track API requests and success rates

### Quick Setup (Dashboard Method - Recommended)
...
2. **Access Integration Hub:**
   ...
   - Click the **"API Integration Hub"** card

3. **Configure Integrations:**
   - Select a category tab (Threat Intel, Geolocation, Notifications, Ticketing, Webhooks)
   - Click **"Configure"** on any integration
   - Enter your API credentials (auto-encrypted on save)
   - Click **"Test"** to verify connectivity
   - Toggle **"Enabled"** to activate

4. **Monitor Usage:**
   - View real-time stats on integration cards
   - Check health status indicators (🟢 Healthy | 🟡 Degraded | 🔴 Error | ⚪ Untested)
   - Export configuration from Settings tab

### Available Integrations (All Free Tier)
...
- IPinfo (50,000/month)
- IP-API (45/min, no key required)
- IPGeolocation (1,000/day)
...
- Discord (Unlimited)
- Telegram (Unlimited)
- Pushover (7,500/month)
- Email/SMTP (Unlimited)
...
The encryption key is **auto-generated** on first run. To verify:
...
- All API credentials are stored encrypted in database, not `.env`

### Getting API Keys

See [docs/API_INTEGRATION_HUB.md](API_INTEGRATION_HUB.md) for detailed instructions on obtaining API keys for each service.

### Settings Management

The Settings tab in Integration Hub provides:

- **🔄 Refresh Data** - Reload integration stats
- **🗑️ Clear Request Logs** - Remove historical logs (with confirmation)
- **🏥 Reset Health Status** - Clear health check results (with confirmation)
- **📥 Export Configuration** - Download config as JSON (credentials excluded)

### Legacy .env Configuration (Not Recommended)

In production, use the **Dashboard UI** instead. For programmatic configuration:
...
mgr.configure_integration(
    'abuseipdb',
    enabled=True,
    api_key='your-api-key-here'  # Auto-encrypted # pragma: allowlist secret
)
```

### Migration from .env to Integration Hub

If you previously used `.env` file for API keys:

1. Use Dashboard UI to configure each integration
2. Comment out all API keys in `.env` file
3. Keep only `IOTSENTINEL_ENCRYPTION_KEY` active
4. Credentials now managed securely via Integration Hub

For complete documentation, see [docs/API_INTEGRATION_HUB.md](API_INTEGRATION_HUB.md)
