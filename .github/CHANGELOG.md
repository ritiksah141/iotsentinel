...
- The post-build verification checked `export-image/rootfs`, which pi-gen **unmounts**
  after writing the `.img`, leaving an empty mountpoint — so every check reported
  "missing" even though the install was complete and the image was fine. It now
  inspects the custom stage's rootfs (`stage-iotsentinel/rootfs`), the real on-disk
  filesystem that is packaged into the image, and logs which rootfs it used.
...
- **setup_pi.sh now completes end-to-end in the arm64 chroot** (all 9 steps: deps,
  venv + pip, DB, sudoers, services enabled) and the image assembles. The remaining
  failure was a bug in the post-build *verification* code, not the image: it located
  the root-owned rootfs with a non-sudo `find | head` pipeline, which returned
  non-zero (permission/SIGPIPE) and aborted the whole build under `set -o pipefail` —
  with no message — after a fully successful ~2h install. The rootfs locate now uses
  `sudo` and is `|| true`-guarded; the image-locate `find | sort | head` got the same
  guard. A test locks both in.
...
- **Fixed the real cause of the empty arm64 image:** in the emulated 64-bit build chroot, apt couldn't verify the Debian base archive keys (`NO_PUBKEY` / "repository is not signed") and couldn't create `/tmp` temp files, so `apt-get update` failed and (under `bash -e`) the deps stage aborted **before installing Zeek/Python/NetworkManager**. The deps stage now ensures `/tmp` is `1777` and writes a build-only apt config that runs apt as root and allows the unsigned base repos (`AllowInsecureRepositories` / `AllowUnauthenticated`) — packages still come from the official Debian/Zeek mirrors, and the override is removed before the image ships, so the device keeps normal apt security. The full dependency set now installs.
...
- **Visual setup flow.** Added a Mermaid flow diagram (download → flash → hotspot → wizard → monitoring) to `docs/SETUP_GUIDE.md`, the README, and the shipped guide.
- **Path A rewritten for non-technical users**, including the explicit "skip every Raspberry Pi Imager customisation prompt - click NO" instruction, and the README points clearly to the full guide and the in-package copy.

### Headless Wi-Fi resilience and image-build fix (2026-06-19)

- **Setup page is reachable over the provisioning hotspot.** The dashboard defaulted `IOTSENTINEL_HOST` to `127.0.0.1`, so on first boot it listened only on localhost and `http://10.42.0.1:8050/setup` never loaded for a device joined to the `IoTSentinel-Setup` hotspot (the wizard only wrote `0.0.0.0` at the end, and only if remote access was enabled). `iotsentinel-dashboard.service` now sets `Environment=IOTSENTINEL_HOST=0.0.0.0` so the dashboard binds all interfaces from first boot.
- **The provisioning hotspot reliably comes up on a headless first boot.** Hardened the three things that silently stop the `IoTSentinel-Setup` AP from appearing: the image now makes NetworkManager the active backend and disables `dhcpcd` (a conflicting manager makes `nmcli` AP creation fail), installs `iw`/`rfkill`, and `setup_hotspot.sh` unblocks the radio (`rfkill unblock wifi`) and sets the regulatory domain (`iw reg set`) before arming. The hotspot is now created as an explicit **open** network (NetworkManager shared mode) instead of `nmcli device wifi hotspot`, which produced a WPA2 network with a generated password the user could never know - the setup network is now genuinely password-free as documented.
